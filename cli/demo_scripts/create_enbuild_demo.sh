...
install_or_upgrade_helm_charts() {
  releaseExist=$(helm list -n enbuild -o json | jq '.[] | select(.name=="enbuild") | .name')
  if [ "$releaseExist" = "" ]
  then
    set -x
    helm upgrade --install --create-namespace ${HELM_DEBUG} --timeout=15m -n enbuild -f values.yaml --atomic \
  ...
  for i in $(seq 1 3); do
    set -x
    helm upgrade --install --create-namespace ${HELM_DEBUG} --timeout=15m -n enbuild -f values.yaml --wait --atomic enbuild vivsoft/enbuild && break
    set +x
...
echo '""""""""""""""""""""""""""""""""""""""""""""'
curl -s -L https://raw.githubusercontent.com/vivsoftorg/enbuild/refs/heads/main/examples/enbuild/quick_install.yaml  >> values.yaml
...
echo ''
echo "Go to https://console.enbuild.com to create your first environment on this cluster '${CLUSTER_NAME}'"
echo '""""""""""""""""""""""""""""""""""""""""""""'